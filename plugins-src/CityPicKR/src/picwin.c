...
// 2026-08-13 2차: 크기를 원본 관례 안에 넣어도(20,181,243 바이트, 원본보다 작다) 그 도시에
// 다시 들어가면 여전히 죽는다. 그러니 크기만의 문제가 아니라 게임 디코더가 우리 인코딩의
// 무언가를 못 받는 것이다(거리·길이 코드 범위가 유력하다 — 원본이 실제로 쓰는 값의 범위를
// 재 보고 그 안으로 맞춰야 한다). 그때까지 다시 잠근다.

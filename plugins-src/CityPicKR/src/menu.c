...
#define ID_PIC_OPEN 0xBF00u   // Trade=0xB10x/0xC0xx, Char=0xB301/0xB310+, Ship=0xB410, Patch=0xB500,
                              // Map=0xB600, Mod=0xB700, QMod=0xB800, Upd=0xB900, Fatigue=0xBA00,
                              // Hotkey=0xBB00, Hint=0xBC00, Market=0xBD00, Save=0xBE0x 과 비충돌
...
static LRESULT CALLBACK SubProc(HWND h, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    if (msg == WM_COMMAND && HIWORD(wp) == 0 && LOWORD(wp) == ID_PIC_OPEN) {
        PicWin_Show(h, g_hinst);
        return 0;
    }
    if (msg == WM_NCDESTROY) {
...
static DWORD WINAPI PicMenuThread(LPVOID param)
{
    (void)param;
    OutputDebugStringW(L"[CityPicKR] menu monitor started.");
    for (;;) {
        HMENU bar;
        g_hwnd = NULL;
        ...
        if (g_hwnd && (bar = GetMenu(g_hwnd)) != NULL) {
            ...
            if (!HasOurMenu(target)) {
                ...
                DrawMenuBar(g_hwnd);
                OutputDebugStringW(L"[CityPicKR] 도시그림 menu installed.");
            }
            ...
            if (g_subHwnd != g_hwnd) {
                ...
                g_subHwnd = g_hwnd;
                OutputDebugStringW(L"[CityPicKR] window subclassed.");
            }
        }
        Sleep(1000);
    }
...
void PicMenu_Init(HINSTANCE hinst)
{
    HANDLE t;
    g_hinst = hinst;
    t = CreateThread(NULL, 0, PicMenuThread, NULL, 0, NULL);
    if (t) CloseHandle(t);
}

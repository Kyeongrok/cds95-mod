...
// ---- 되쓰기 전에 원본 파트의 "범위" 를 반드시 맞출 것 ----
// 게임 디코더는 원본 인코더가 내는 범위까지만 받는다. CITYCG.CDS(도시 그림)를 넣다가
// 두 번 죽였고, 둘 다 형식이 아니라 범위 문제였다.
//   1차  크기를 넘김 — 팔레트 파트가 원본에서 226장 전부 258바이트(압축크기 == 원본크기 =
//        무압축 저장)인데 압축 형식 371바이트로 썼고, 그림도 원본 최대 113,474 → 122,790.
//   2차  크기는 원본보다 작은데 거리 코드를 16,382 까지 씀(원본은 8,191 까지) → 그 도시에
//        들어가면 죽었다. 게임 디코더가 13비트까지만 받는 것으로 보인다.
// 게임 파일들을 훑어 보면 거리 8,191 · 길이 512 · 코드 8,447 이 한결같은 상한이다
// (CITYCG · EVANIME · ITEM · MPCG · DISCOVER 모두 같고, MALE 은 그보다 좁다).
// 그래서 이 인코더는 그 안에서만 낸다(ls12.c 의 LZ_WINDOW / LZ_MAX_MATCH — 늘리지 말 것).
//
// 새 파일에 쓸 때는 그 파일 파트들의 압축크기 분포와 Ls12_PartMaxCode 를 먼저 재 보고,
// 새 파트가 그 범위를 넘지 않는지 쓰기 전에 확인하라(CityPicKR/src/citycg.c 가 그렇게 한다).
...
// 그 파트의 압축 스트림에 나오는 가장 큰 코드값. 무압축 파트나 실패면 0.
// 게임 디코더가 받는 코드 범위는 원본 파일이 실제로 쓰는 범위까지로 봐야 한다 —
// 넣을 파트가 원본 파트들보다 큰 코드를 쓰면 게임이 죽는다(ls12.c 의 LZ_MAX_CODE 주석 참고).
// 되쓰기 전에 이 값으로 원본 파트들과 견줘 보는 것이 크기 검사보다 확실하다.

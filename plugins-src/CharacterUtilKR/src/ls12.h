...
// ---- 되쓰기 전에 파트 크기를 반드시 확인할 것 ----
// 아래 인코더는 실제로 압축하지 않는다(바로 아래 설명 참고). 그래서 새로 쓴 파트가
// 원본보다 커지는데, 게임은 파일마다 그 파일의 관례에 맞춘 버퍼로 읽는 듯하다.
// CITYCG.CDS(도시 그림)에서 이걸로 게임을 죽였다 —
//   팔레트 파트가 원본에서 226장 전부 258바이트(압축크기 == 원본크기 = 무압축 저장)인데
//   압축 형식 371바이트로 써넣었고, 그림 파트도 원본 최대 113,474 를 넘겨 122,790 이 됐다.
// 얼굴(MALE/FEMALE.CDS)은 파트가 7,680바이트라 불어나도 게임이 삼켰다.
// 새 파일에 쓰기 전에 그 파일 파트들의 압축크기 분포를 먼저 보고, 넘길 것 같으면
// 인코더에 LZ 매치를 넣거나 무압축 저장(압축크기 = 원본크기)으로 맞춰야 한다.

...
매매      교역소에서 그림과 함께 담아서 한 번에 사고판다
지도      함선      워프 >
모드 >    플러그인 관리 · 퀘스트 모드 · 패치
피로도    줄일 값(기본 20)을 적고 [줄이기]
규율      규율(0~100) · 날짜와 눈금 · 지금 칸 지형 · 항해술/운용술 · 오늘 깎일 폭 · 바뀐 자리 기록
          [화면 왼쪽 위에 규율 보이기] 를 켜면 게임 화면에도 규율 · 운 · 신앙, 성격(델포이 계시와 같은 낱말) · 여급궁합이 뜬다(WindArrowKR 가 그린다)
단축키    글자 한 개로 위 창들을 연다(기본 정보=I, 스폰서=P, 매매=E …). 줄을 고르고 키를 누르면 바뀐다
업데이트
```

단축키는 `CDS95Util\hotkeys.json` 에 남습니다. 게임이 글자키를 받는 자리(이름 적기 같은 것)에서는
[단축키 켜기] 를 꺼 두면 됩니다. Ctrl · Alt 조합과 글자 입력칸은 처음부터 건드리지 않습니다.
